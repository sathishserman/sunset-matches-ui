...
import { loadCommunities, setCommunities, updateUserSelections } from '../../redux/actions';
...
export default function Communities({ navigation }: { navigation: any }) {
  ...
  const { communities, userSelections } = useSelector((state: RootState) => state.communitiesState);
  ...
  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity onPress={() => handleSelect(item.id)} style={styles.communityButton}>
      <Image source={{ uri: item.imageUrl }} style={styles.communityImage} />
      {selectedCommunities.includes(item.id) && <View style={styles.selectedOverlay} />}
    </TouchableOpacity>
  );
